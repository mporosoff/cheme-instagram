...
When an article exposes a Newscenter featured image, graphical-abstract/publication metadata, a labeled graphical-abstract or TOC figure, structured article data, or a social-sharing image, the script downloads the best available candidate into a private **UR ChemE IG Discovery Media** Drive folder. It records the article URL, direct image URL, image type, and best available creator/site credit in the row. Content Studio automatically loads the saved image, labels it as ready, uses it in the graphic, and adds the recorded image credit to the draft caption. The upload control remains available to replace it.
