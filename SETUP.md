...
In the Studio, **AI Generate** offers three routes: prepare a prompt and upload an image made elsewhere; create a simple illustrated diagram using the existing Claude API key; or generate an image with an OpenAI image API key. Prompt preparation needs no key or API call. The other two routes incur the selected provider’s API usage; image generation is separate from a ChatGPT subscription. OpenAI currently uses `gpt-image-2.5-flare`, one JPEG per request, with draft/standard/detailed quality. Add both provider keys under **API keys** in the Studio header and click **Save keys**. **Remember both keys in this browser** is on by default and retains them after closing or restarting the browser; turn it off for tab-only storage. Existing keys populate the shared panel. To remove one key, clear its field and save; **Clear both fields** followed by **Save keys** removes both. Cancelling makes no changes. AI Generate automatically uses the saved key, and its **Manage API keys** button returns to the current prompt and preview after saving or cancelling. Clearing browser data removes saved keys. Requests go directly from the browser to the selected provider, using the paper facts and creative prompt. A generated image is only a candidate until **Use image & continue to caption** is selected for a paper, and publication still requires **Approve**. Check illustrations against the paper before publishing. Image-only templates export at 1080×1350 or 1080×1080 without institutional overlays.
